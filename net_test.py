...
def generate_solar_data(input_url, time_steps, normalize=1, val_size=0.1, test_size=0.1):
    ...
    cache_file = os.path.join(cache_path, "net_test.csv")
    ...
    for i, day in enumerate(per_day):
        total = day["net.max"].values
        #print(len(total))
        current_set = "test"
        max_total_for_day = np.array(day["net.max"].values[0])
...
z = load_model("./Model/net_train.cntk")
z = z(x)
