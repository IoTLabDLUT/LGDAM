...
def generate_solar_data(input_url, time_steps, normalize=1, val_size=0.1, test_size=0.1):
    ...
    cache_file = os.path.join(cache_path, "cpu_test.csv")
...
z = load_model("./Model/cpu_train.cntk")
z = z(x)
...
for j, ds in enumerate(["val", "test"]):
    ...
    #np.savetxt('./datatest/craw.csv', Y[ds])
    a[j].plot(np.array(results) * NORMALIZE, label=ds + ' pred')
    #np.savetxt('./datatest/cpred.csv', np.array(results))
    a[j].legend()
